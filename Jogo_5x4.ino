...
#define NUM_JOGADORES 5 //Define o número de jogadores em 5

// uint8_t byte_jogador[0];
// uint8_t byte_jogador[1];
// uint8_t byte_jogador[2];
// uint8_t byte_jogador[3];
// uint8_t byte_jogador[4];
// uint8_t byte_jogador[5];
uint8_t byte_jogador[NUM_JOGADORES+1];


// uint8_t botoes_jogador[0];
// uint8_t botoes_jogador[1];
// uint8_t botoes_jogador[2];
// uint8_t botoes_jogador[3];
// uint8_t botoes_jogador[4];
// uint8_t botoes_jogador[5];
uint8_t botoes_jogadores[NUM_JOGADORES+1];

// uint8_t resposta_jogador[0];
// uint8_t resposta_jogador[1];
// uint8_t resposta_jogador[2];
// uint8_t resposta_jogador[3];
// uint8_t resposta_jogador[4];
uint8_t respostas[NUM_JOGADORES];

// uint8_t pontuacao_jogador[0];
// uint8_t pontuacao_jogador[1];
// uint8_t pontuacao_jogador[2];
// uint8_t pontuacao_jogador[3];
// uint8_t pontuacao_jogador[4];
uint8_t pontuacao_jogadores[NUM_JOGADORES];
...
void setup() {
  ...
  // pontuacao_jogador[1] = 0;
  // pontuacao_jogador[2] = 0;
  // pontuacao_jogador[3] = 0;
  // pontuacao_jogador[4] = 0;
pontuacao_jogador[0,0,0,0,0];
}
...
void loop() {
  ...
  Wire.requestFrom(ENDERECO_PCF8574_PLAYER_0, 1);  // Solicita 1byte do dispositivo escravo
  byte_jogador[0] = Wire.read();                     // Salva na variavel o byte enviado pelo dispositivo
  botoes_jogador[0] = byte_jogador[0] & 0b00111111; //realiza uma máscara binária entre o byte lido e o valor  0b00111111
  // Resposta a:
  if (botoes_jogador[0] == 0b111110) {
    Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_0);
    Wire.write(0b01111111);  // Envia o byte ao dispositivo
    Wire.endTransmission();  // Finaliza a comunicacao com o dispositivo i2c
    resposta_certa = byte_jogador[0] & 0b00001111;
  }
  // Resposta b:
  if (botoes_jogador[0] == 0b111101) {
    Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_0);
    Wire.write(0b01111111);  // Envia o byte ao dispositivo
    Wire.endTransmission();  // Finaliza a comunicacao com o dispositivo i2c
    resposta_certa = byte_jogador[0] & 0b00001111;
  ...
  // Finaliza rodada:
  if (botoes_jogador[0] == 0b101111)  // Se pressionou o botao que finaliza a rodada
  {
    Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_0);
  ...
  //Inicia a rodada:
  if (botoes_jogador[0] == 0b011111)  // Se pressionou o botao que inicia a rodada
  {
    Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_0);
    Wire.write(0b11111111);  // Envia o byte ao dispositivo
    Wire.endTransmission();  // Finaliza a comunicacao com o dispositivo i2c
  ...
  if (iniciar_rodada == 1) {
    ...
    byte_jogador[1] = Wire.read();                     // Salva na variavel o byte enviado pelo dispositivo
    botoes_jogador[1] = byte_jogador[1] & 0b00001111;
    if (botoes_jogador[1] == 0b1110) {// Se o jogador 1 escolher a letra a)
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_A);
      Wire.write(0b11101111);                           // Envia o byte correspondente a letra a) ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[1]);                    // Imprime no terminal
      Serial.println("Jogador A apertou a opcao 'a'");  // Imprime no terminal
      resposta_jogador[0] = botoes_jogador[1];
    } else if (botoes_jogador[1] == 0b1101) { // Se o jogador 1 escolher a letra b)
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_A);
      ...
      resposta_jogador[0] = botoes_jogador[1];
    } else if (botoes_jogador[1] == 0b1011) { // Se o jogador 1 escolher a letra c)
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_A);
      Wire.write(0b10111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[1]);                    // Imprime no terminal
      ...
      resposta_jogador[0] = botoes_jogador[1];
    } else if (botoes_jogador[1] == 0b0111) {// Se o jogador 1 escolher a letra d)
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_A);
      Wire.write(0b01111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[1]);                    // Imprime no terminal
    ...
    byte_jogador[2] = Wire.read();                     // Salva na variavel o byte enviado pelo dispositivo
    botoes_jogador[2] = byte_jogador[2] & 0b00001111;
    if (botoes_jogador[2] == 0b1110) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_B);
      Wire.write(0b11101111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[2]);                    // Imprime no terminal
      Serial.println("Jogador B apertou a opcao 'a'");  // Imprime no terminal
      resposta_jogador[1] = botoes_jogador[2];
    } else if (botoes_jogador[2] == 0b1101) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_B);
      Wire.write(0b11011111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[2]);                    // Imprime no terminal
      ...
      resposta_jogador[1] = botoes_jogador[2];
    } else if (botoes_jogador[2] == 0b1011) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_B);
      Wire.write(0b10111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[2]);                    // Imprime no terminal
      ...
      resposta_jogador[1] = botoes_jogador[2];
    } else if (botoes_jogador[2] == 0b0111) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_B);
      Wire.write(0b01111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[2]);                    // Imprime no terminal
    ...
    byte_jogador[3] = Wire.read();                     // Salva na variavel o byte enviado pelo dispositivo
    botoes_jogador[3] = byte_jogador[3] & 0b00001111;
    if (botoes_jogador[3] == 0b1110) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_C);
      Wire.write(0b11101111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[3]);                    // Imprime no terminal
      Serial.println("Jogador C apertou a opcao 'a'");  // Imprime no terminal
      resposta_jogador[2] = botoes_jogador[3];
    } else if (botoes_jogador[3] == 0b1101) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_C);
      Wire.write(0b11011111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[3]);                    // Imprime no terminal
      ...
      resposta_jogador[2] = botoes_jogador[3];
    } else if (botoes_jogador[3] == 0b1011) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_C);
      Wire.write(0b10111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[3]);                    // Imprime no terminal
      ...
      resposta_jogador[2] = botoes_jogador[3];
    } else if (botoes_jogador[3] == 0b0111) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_C);
      Wire.write(0b01111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[3]);                    // Imprime no terminal
    ...
    byte_jogador[4] = Wire.read();                     // Salva na variavel o byte enviado pelo dispositivo
    botoes_jogador[4] = byte_jogador[4] & 0b00001111;
    if (botoes_jogador[4] == 0b1110) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_D);
      Wire.write(0b11101111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[4]);                    // Imprime no terminal
      Serial.println("Jogador D apertou a opcao 'a'");  // Imprime no terminal
      resposta_jogador[3] = botoes_jogador[4];
    } else if (botoes_jogador[4] == 0b1101) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_D);
      Wire.write(0b11011111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[4]);                    // Imprime no terminal
      ...
      resposta_jogador[3] = botoes_jogador[4];
    } else if (botoes_jogador[4] == 0b1011) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_D);
      Wire.write(0b10111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[4]);                    // Imprime no terminal
      ...
      resposta_jogador[3] = botoes_jogador[4];
    } else if (botoes_jogador[4] == 0b0111) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_D);
      Wire.write(0b01111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[4]);                    // Imprime no terminal
    ...
    byte_jogador[5] = Wire.read();                     // Salva na variavel o byte enviado pelo dispositivo
    botoes_jogador[5] = byte_jogador[5] & 0b00001111;
    if (botoes_jogador[5] == 0b1110) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_E);
      Wire.write(0b11101111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[5]);                    // Imprime no terminal
      Serial.println("Jogador E apertou a opcao 'a'");  // Imprime no terminal
      resposta_jogador[4] = botoes_jogador[5];
    } else if (botoes_jogador[5] == 0b1101) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_E);
      Wire.write(0b11011111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[5]);                    // Imprime no terminal
      ...
      resposta_jogador[4] = botoes_jogador[5];
    } else if (botoes_jogador[5] == 0b1011) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_E);
      Wire.write(0b10111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[5]);                    // Imprime no terminal
      Serial.println("Jogador E apertou a opcao 'c'");  // Imprime no terminal
      resposta_jogador[4] = botoes_jogador[4];
    } else if (botoes_jogador[5] == 0b0111) {
      Wire.beginTransmission(ENDERECO_PCF8574_PLAYER_E);
      Wire.write(0b01111111);                           // Envia o byte ao dispositivo
      Wire.endTransmission();                           // Finaliza a comunicacao com o dispositivo i2c
      Serial.println(byte_jogador[5]);                    // Imprime no terminal
  ...
  if (finalizar_rodada == 1) {
    ...
    // ***********************************************************************************************************
    if (resposta_jogador[0] == resposta_certa) {

      pontuacao_jogador[0] += 1;
      lcd.setCursor(7, 4);  // Coluna, Linha
      lcd.print(pontuacao_jogador[0]);
      resposta_jogador[0] = 0;
    }
    if (resposta_jogador[1] == resposta_certa) {

      pontuacao_jogador[1] += 1;
      lcd.setCursor(9, 4);  // Coluna, Linha
      lcd.print(pontuacao_jogador[1]);
      resposta_jogador[1] = 0;
    }
    if (resposta_jogador[2] == resposta_certa) {

      pontuacao_jogador[2] += 1;
      lcd.setCursor(11, 4);  // Coluna, Linha
      lcd.print(pontuacao_jogador[2]);
      resposta_jogador[2] = 0;
    }
    if (resposta_jogador[3] == resposta_certa) {

      pontuacao_jogador[3] += 1;
      lcd.setCursor(13, 4);  // Coluna, Linha
      lcd.print(pontuacao_jogador[3]);
      resposta_jogador[3] = 0;
    }
    if (resposta_jogador[4] == resposta_certa) {

      pontuacao_jogador[4] += 1;
      lcd.setCursor(15, 4);  // Coluna, Linha
      lcd.print(pontuacao_jogador[4]);
      resposta_jogador[4] = 0;
    }
    reset_leds = 1;
    finalizar_rodada = 0;
  }
